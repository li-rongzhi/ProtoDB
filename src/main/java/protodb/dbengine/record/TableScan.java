...
import protodb.dbengine.query.Constant;
import protodb.dbengine.query.UpdateScan;
import protodb.dbengine.xact.Transaction;

import static java.sql.Types.INTEGER;

// scan thru the whole table
public class TableScan implements UpdateScan {
    private Transaction tx;
    private Layout layout;
    private RecordMgr rp;
    ...
    public TableScan(Transaction tx, String tblname, Layout layout) {
        ...
        if (tx.size(filename) == 0) {
            System.out.println("Move to new block");
            moveToNewBlock();
        }
        else
            moveToBlock(0);
    ...
    public void moveToRid(RID rid) {
        ...
        rp = new RecordMgr(tx, blk, layout);
